...
class FluxLogger:
    def __init__(self, rules: dict = {}):
        ...
        self._default_rules: dict = {
            ...
            "metadata": {
                ...
                "include_level_name": True,  # If True, include the level name in metadata
        ...
        if self.rs_formatting["enable_placeholders"]:
            self.__log("warning", "Placeholders are currently not supported. You're seeing this message because you have the Formatting > Enable Placeholders rule set to True.")
            self.__reset_timestamp()

    def __reset_timestamp(self):
        self._blank_timestamp: str = f"\033[{
            len(datetime.datetime.now().strftime(self.rs_timestamps["format"]))}C"
        self._previous_timestamp: str = None

    def __log(self, level: str, message: any, metadata: dict = {}, **kwargs):
        ### CONDITIONS TO MEET ###
        ...
        if any(sub in message.lower() for sub in [substring.lower() for substring in self.rs_filtering["exclude_messages"]]):
            return

        if self.rs_filtering["include_only_messages"] and not any(sub in message.lower() for sub in [substring.lower() for substring in self.rs_filtering["include_only_messages"]]):
            return
        ...
        context = {
            'tms': self.__define_timestamp() if terminal_width > 75 else self.__define_timestamp(format="%H:%M:%S"),
            ...
            'lvl': self.levels[level]["name"]["display"].upper() if terminal_width > 75 else self.levels[level]["name"]["short"][:3].upper(),
            'message': message
        }

        log_line_format = self.rs_log_line["format"] + ".[message]"
        formatted_message = self.__parse_placeholders(log_line_format, context, level)
        ...
        if self.rs_metadata["show_metadata"]:
            message_length = shutil.get_terminal_size().columns - len(self.__remove_ansi(
                formatted_message.replace(context["tms"], self.__define_timestamp(no_blank=True))))

            if self.rs_metadata["include_timestamp"]:
                self._log_metadata = self.__define_m_widget(f"[tms: {self.__define_timestamp(
                    no_blank=True)}]", message_length - len(self._log_metadata)) + self._log_metadata
            if self.rs_metadata["include_level_name"]:
                self._log_metadata = self.__define_m_widget(
                    f"[lvl: {level}]", message_length - len(self._log_metadata)) + self._log_metadata
            if self.rs_metadata["include_thread_name"]:
                self._log_metadata = self.__define_m_widget(
                    f"[thr: {threading.current_thread().name}]", message_length - len(self._log_metadata)) + self._log_metadata
            ...
            if self.rs_metadata["include_file_name"]:
                self._log_metadata = self.__define_m_widget(
                    f"[fl: {file_name}]", message_length - len(self._log_metadata)) + self._log_metadata
            if self.rs_metadata["include_wrapping_function"]:
                self._log_metadata = self.__define_m_widget(
                    f"[wfc: {wrapping_func}]", message_length - len(self._log_metadata)) + self._log_metadata
            if self.rs_metadata["include_function"]:
                self._log_metadata = self.__define_m_widget(
                    f"[fnc: {function}]", message_length - len(self._log_metadata)) + self._log_metadata
            if self.rs_metadata["include_line_number"]:
                self._log_metadata = self.__define_m_widget(
                    f"[ln: {line_number}]", message_length - len(self._log_metadata)) + self._log_metadata

            self._log_metadata = f"{Style.RESET_ALL}{Style.DIM}{
                self._log_metadata.rjust(message_length)}"

        formatted_message += self._log_metadata

        ### STACKING CONDITIONALS & self._previous_... VARIABLES & LOGGING ###

        if (
            f"{message if self.rs_stacking["case_sensitive"] else message.lower()}" == f"{
            ...
            lines = textwrap.wrap(f"{message}", width=shutil.get_terminal_size().columns + self._prompt_length*3, initial_indent="\033[1C"*self._prompt_length, subsequent_indent="\033[1C"*self._prompt_length)

            self._last_line_height = len(lines)

            if len(lines) > 1:
                print(f"{formatted_message.replace(context["message"], "")}")
                print("\033[2A")
                for line in lines:
                    self._last_line_length = len(line.replace("\033[1C", " "))
                    print(f"{self.levels[level]["color"] if self.rs_formatting["color_enabled"] else ""}{line}")
            else:
                self._last_line_length = len(lines[0].replace("\033[1C", " "))
                print(formatted_message)

    def __close(self):
        terminal_width = shutil.get_terminal_size().columns
        timestamp = self.__define_timestamp() if terminal_width > 75 else self.__define_timestamp(format="%H:%M:%S")
        self._previous_timestamp = datetime.datetime.now().strftime(
            self.rs_timestamps["format"])

        if self._previous_message is not None and self._log_count > 1:
            print(f"\033[{self._last_line_height}A{Fore.GREEN}{timestamp}{Style.RESET_ALL}\033[{self._last_line_height - 1}B\033[{self._last_line_length}C {Style.DIM}{Fore.WHITE}(x{self._log_count}){Style.RESET_ALL}")
            ...
    ...
    def __parse_placeholders(self, format_str, context, level):
        ...
        def get_parts(param, defaults, split_char='_'):
            ...
            return [parts[i] if i < len(parts) else defaults[i] for i in range(len(defaults))]
        ...
        def to_str(value, default, param_name):
            try:
                ...
                print(f"The {param_name} parameter couldn't be properly applied because the values of one of it's options couldn't be converted to an string. Instead, {default} will be supplied.")
                return default
        ...
        def to_int(value, default, param_name):
            try:
                ...
                print(f"The {param_name} parameter couldn't be properly applied because the values of one of it's options couldn't be converted to an integer. Instead, {default} will be supplied.")
                return default
        ...
        def apply_parameters(value, parameters):
            ...
            for param in parameters:
                ...
                if param.startswith('Z_'):
                    pad_length = to_int(param[2:], default_pad_length, param) if len(param) > 2 else default_pad_length
                    ...
                    stripped_value = stripped_value[:(width - len(ending))] + ending if len(stripped_value) > width else stripped_value

                elif param.startswith('c_') and self.rs_formatting["color_enabled"]:
                        color = param.split('_', 1)[1]
            
            if original_value == context.get('tms', ''):
                self._prompt_length += len(str(self.__define_timestamp(no_blank=True) if terminal_width > 75 else self.__define_timestamp(format="%H:%M:%S", no_blank=True)))
            elif original_value != context.get('message', ''):
            ...
            for match in ansi_escape.finditer(original_value):
                ...
                recolored_value += stripped_value[stripped_index:start - stripped_index]
        ...
        def placeholder_replacer(match):
            ...
            parameters = match.group(2).strip(
                '()').split(',') if match.group(2) else []
            
            if placeholder == "ln" and not any(parameter.startswith("L_") for parameter in parameters):
            ...
            for parameter in range(len(parameters)):
                if parameters[parameter].startswith("c_"):
                    parameters[parameter] = f"c_{c_map[parameters[parameter][2:] or -1]}"
            ...
            c_lvl_map = {
                ...
                'message': self.levels[level]["color"]
            }
            color = c_lvl_map.get(placeholder, '')
            if color and not any(parameter.startswith("c_") for parameter in parameters) and self.rs_formatting["color_enabled"]:
        ...
        self._prompt_length = sum(len(match.group()) for match in inverse_pattern.finditer(str(format_str))
    ...
    def __define_timestamp(self, no_blank: bool = False, format: str = None) -> str:
        timestamp = datetime.datetime.now(
            datetime.UTC if self.rs_timestamps["use_utc"] else None).strftime(format or self.rs_timestamps["format"])
        if no_blank:
            return timestamp
        if timestamp == self._previous_timestamp and not self.rs_timestamps["always_show"]:
            timestamp = self._blank_timestamp
        return timestamp

    def __log_method(self, message, level):
        # Get current frame
        ...
        self.__log(level, message, metadata)
        self.__close()

    def info(self, message, **kwargs):
        self.__log_method(message, self.levels["info"]["name"]["display"])

    def debug(self, message, **kwargs):
        self.__log_method(message, self.levels["debug"]["name"]["display"])

    def warning(self, message, **kwargs):
        self.__log_method(message, self.levels["warning"]["name"]["display"])

    def success(self, message, **kwargs):
        self.__log_method(message, self.levels["success"]["name"]["display"])

    def error(self, message, **kwargs):
        self.__log_method(message, self.levels["error"]["name"]["display"])

    def critical(self, message, **kwargs):
        self.__log_method(message, self.levels["critical"]["name"]["display"])
